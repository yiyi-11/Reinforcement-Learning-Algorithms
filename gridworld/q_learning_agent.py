...
class QLearningAgent(AgentGrid):
    def __init__(self, env: Env):
        super(QLearningAgent, self).__init__(env)  # agent环境初始化
        self.reward_episode = []
        self.time_episode = []
    ...
    def learning(self, gamma, alpha, max_episode_num, render):
        ...
        while num_episode < max_episode_num:  # 设置终止条件
            ...
            while not is_done:  # 针对一个Episode内部
                ...
                # get new Q(s, a) <- Q(s, a) + alpha*(reward + gamma*maxQ(s', *) - Q(s, a))
                old_q = self._get_Q(s0, a0)
                max_q = self._get_max_Q(s1)
                td_target = r1 + gamma * max_q
                new_q = old_q + alpha * (td_target - old_q)
                self._set_Q(s0, a0, new_q)  # 更新Q(s, a)
...
if __name__ == "__main__":
    ...
    np.save(file=f'data/{env_name}_qlearn_reward.npy', arr=np.array(agent.reward_episode))
    np.save(file=f'data/{env_name}_qlearn_step.npy', arr=np.array(agent.time_episode))
