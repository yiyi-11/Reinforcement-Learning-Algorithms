...
class DoubleDQN(object):
    ...
    def train_one_step(self):
        ...
        reward_batch = torch.tensor(reward_batch, device=self.device, dtype=torch.float).unsqueeze(1)  # tensor([1., 1.,...,1])
        new_state_batch = torch.tensor(new_state_batch, device=self.device, dtype=torch.float)
        is_done_batch = torch.tensor(np.float32(is_done_batch), device=self.device).unsqueeze(1)
        q_values = self.policy_model.forward(state_batch).gather(dim=1, index=action_batch)

        max_next_q_action = torch.argmax(self.policy_model.forward(new_state_batch).detach(), dim=-1).unsqueeze(1)
        q_max_next = self.target_model.forward(new_state_batch).gather(dim=1, index=max_next_q_action).detach()

        target_values = reward_batch + self.gamma * q_max_next * (1 - is_done_batch)
...
if __name__ == "__main__":
    ...
    np.save(file=f'data/{env_name}_doubledqn_score.npy', arr=np.array(score_list))
    np.save(file=f'data/{env_name}_doubledqn_ma_reward.npy', arr=np.array(moving_average_reward_list))
